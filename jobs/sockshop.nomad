job "sockshop" {
  ...
  group "user" {
    ...
    task "user" {
      ...
      template {
        data = <<EOH
        MONGO_PASS="{{with secret "secrets/sockshop/databases/userdb" }}{{.Data.pwd}}{{end}}"
        EOH
  ...
  group "catalogue" {
    ...
    task "cataloguedb" {
      ...
      template {
        data = <<EOH
	      MYSQL_ROOT_PASSWORD="{{with secret "secrets/sockshop/databases/cataloguedb" }}{{.Data.pwd}}{{end}}"
        EOH
