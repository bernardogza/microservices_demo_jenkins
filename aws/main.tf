...
resource "null_resource" "start_sock_shop" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "nomad job run -address=http://${module.nomadconsul.primary_server_private_ips[0]}:4646 /home/ubuntu/jenkins-java.nomad"
    ]
    ...
    connection {
      ...
      agent = false
      user = "ubuntu"
      private_key = "${file("/Users/bernardogza/Keys/bernardokey.pem")}"
    }
  }
}
